  ...
  return (
    <div className="flex-wrap items-center justify-between py-10 space-y-14 sm:flex sm:space-y-0">
      <div>
        ...
        {/* ✅ Fixed tag overflow + wrap */}
        <div className="flex flex-wrap gap-2 mt-2 text-[#F719CC] max-w-full overflow-hidden">
          {tags.map((tag, index) => (
            <span
              key={index}
              className="px-2 py-1 text-sm font-medium bg-white/5 rounded"
            >
              {tag.name}
            </span>
          ))}
